...
//  i2s_set_sample_rates((i2s_port_t)m_i2s_num, srate);
    xiaogyan_audio_set_sample_rate(srate);
    return true;
}
//---------------------------------------------------------------------------------------------------------------------
...
//  i2s_set_sample_rates((i2s_port_t)m_i2s_num, sampRate);
    xiaogyan_audio_set_sample_rate(sampRate);
...
//      return false;
//  }
    uint32_t data1 = (s32 >> 24) & 0xFF;
    uint32_t data2 = (s32 >>  8) & 0xFF;
    uint8_t data = (uint8_t)((data1 + data2) / 2);
    bool ret = xiaogyan_audio_write(data, 100);
    if(ret == false){
        return false;
    }
